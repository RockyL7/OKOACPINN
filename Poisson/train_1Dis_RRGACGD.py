...
path = f"Poisson/output/{trial}_dis_34001RRGACGD_tol_{tol}_atol_{atol}_g_iter_{g_iter}_term_min_{term_min}_rr_lambda_{rr_lambda}_init_prob_{init_prob}_warm_{warm_start}"
...
optimizer = CGDS.RRGACGD(x_params = D_GACGD.parameters(), 
                           y_params = PINNGACGD.parameters(), lr_x=lr, lr_y=lr, tol=tol, atol = atol, eps=1e-8, beta=0.99, track_cond = track_cond, term_min = term_min, rr_lambda = rr_lambda, init_prob = init_prob, warm_start = warm_start)           
