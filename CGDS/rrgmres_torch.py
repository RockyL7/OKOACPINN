...
def RRGMRES(Avp,              # Linear operator
    ...
    cdf_upper = 1 - (1 - rr_lambda) ** (max_iter - term_min)
    dis = torch.distributions.geometric.Geometric(torch.tensor([rr_lambda]))
    term = dis.sample()
    while term > max_iter - term_min-1:
        term = dis.sample()
    term += term_min
    ...
    for j in range(max_iter):
        if (j < term_min):
            ...
            sol = x0 + V_s.T @ y.squeeze(-1)
            amp = sol - prev_sol
            cdf_curr = 1 - (1 - rr_lambda) ** (j - term_min)
            weight = 1 / (1 - cdf_curr / cdf_upper)
            sol_b.add(init_weight * weight * amp)
            # if residual < tol * bnorm or residual < atol:
            #     return sol_b, sol, (j, err_history)
            if term <= j:
                return sol_b, sol, (j+1, err_history)
            prev_sol = sol
    return sol_b, sol, (j, err_history)