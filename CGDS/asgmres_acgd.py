...
from .asgmres_torch import ASGMRES
...
class ASGACGD(object):
    ...
    def step(self, loss_x, loss_y, trigger=None):
        ...
        #print("gx", torch.norm(grad_gx_vec))
        #print("fy", torch.norm(grad_fy_vec))
        ...
        soln, uw_soln, (num_iter, err_history) = ASGMRES(Avp=Avp, b=RHS, x0=prev_x0,
                                              max_iter=self.max_iter,
                                              tol=tol, atol=atol, eta = self.eta,
                                              track=track_flag)
                
        # print("sol", soln)
        #print("sol_norm", torch.norm(soln))
        self.state['x0'] = uw_soln
        self.state.update(
            {
