...
parser.add_argument('-eta','-eta', help='deterministic steps', type = float, required=True)
...
print(PINN_GACGD)
# print("rand1", PINN_GACGD.x_ic)
# print("rand2", PINN_GACGD.t_bc)



path = f"AC/output/{trial}-new_adaptive_{adaptiveTol}_1dis_ASGACGD_lrmax_{lr_max}_lrmin_{lr_min}_PINN_{layers[1]}_{layers[2]}_dis_{dis_layers[1]}_{dis_layers[2]}_eta_{eta}_warm_{warm_start}"
...
GACGD_optimizer = CGDS.ASGACGD(x_params = discriminator.parameters(), 
                           y_params = PINN_GACGD.parameters(), max_iter = 1000, lr_x=lr_max, lr_y=lr_min, tol=1e-7, atol = 1e-20, eps=1e-8, beta=0.99, track_cond = lambda x, y: True, eta = eta, warm_start = warm_start)
