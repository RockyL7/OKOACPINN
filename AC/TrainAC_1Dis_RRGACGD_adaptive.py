...
parser.add_argument('-term_min','-term_min', help='deterministic steps', type = int, required=True)
parser.add_argument('-rr_lambda','-rr_lambda', help='temperature parameter', type = float, required=True)
parser.add_argument('-init_prob','-init_prob', help='initial truncation prob', type = float, required=True)
...
trial = args.trial
# print("term_min", term_min)
# print("rr_lambda", rr_lambda)
...
path = f"AC/output/{trial}-new_adaptive_{adaptiveTol}_1dis_RRGACGD_lrmax_{lr_max}_lrmin_{lr_min}_PINN_{layers[1]}_{layers[2]}_dis_{dis_layers[1]}_{dis_layers[2]}_term_min_{term_min}_rr_lambda_{rr_lambda}_init_prob_{init_prob}_warm_{warm_start}"
...
GACGD_optimizer = CGDS.RRGACGD(x_params = discriminator.parameters(), 
                           y_params = PINN_GACGD.parameters(), max_iter = 1000, lr_x=lr_max, lr_y=lr_min, tol=1e-7, atol = 1e-20, eps=1e-8, beta=0.99, track_cond = lambda x, y: True, term_min = term_min, rr_lambda = rr_lambda, init_prob = init_prob, warm_start = warm_start)
...
for i in range(max_iter):
    ...
    # print(i, GACGD_optimizer.info["num_iter"])
